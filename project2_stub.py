...
import enum, math, random, time, tkinter, sys
...
def euclidean_mst(points):
    ...
    #debug purpose
    #print(edges[0][0].x,edges[0][0].y,edges[0][1].x,edges[0][1].y,edges[0][2])

    #prim-jarnik algorithm
    start = edges[0][0] #initial start spanned
    spanned = [start]
    tree = []

    #number of edges for mst is n-1 where n is number of vertices
    while len(tree) < (len(points)-1):

        #variable that will store edge with lowest weight
        b = None
        #loop through every edge for graph
        for e in edges:
            #check if vertice is in spanned and exclusive or 
            if e[0] in spanned and e[1] not in spanned 
            or e[1] in spanned and e[0] not in spanned:
...
def main():
    if len(sys.argv) != 2:
        print("usage: ", sys.argv[0], " <number of points>")
    else:
        mst_trial(int(sys.argv[1]))
